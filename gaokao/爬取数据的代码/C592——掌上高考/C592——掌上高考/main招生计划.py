...
for name in nameList:
    ...
    for year in years:
        with open(f"Data/{name}--{year}.csv", "w", newline="", encoding="UTF-8") as csvfile:
            writer = csv.writer(csvfile)
            # 先写入columns_name
            writer.writerow(["年份", "学校", "专业名称", "计划招生", "学制", "学费", "选科要求"])
        ...
        for local_type_id in local_type_id_list:
            for pn in range(1, page):
                ...
                for item in items:
                    ...
                    writerData = []
                    while True:
                        j = f'api.zjzw.cn/web/api/?local_batch_id={local_batch_id}&local_province_id={local_province_id}&local_type_id={local_type_id}&page={ItemPage}&school_id={school_id}&size=10&special_group=&uri=apidata/api/gkv3/plan/school&year={year}'
                        numNowFound = ItemPage * size
                        signsafe = ctx.call('v', data_dict, j)
                        itemUrl = f'https://api.zjzw.cn/web/api/?local_batch_id={local_batch_id}&local_province_id={local_province_id}&local_type_id={local_type_id}&page={ItemPage}&school_id={school_id}&size=10&special_group=&uri=apidata/api/gkv3/plan/school&year={year}&signsafe={signsafe}'
                        while True:
                            try:
                        ...
                        if numFound == 0:
                            ...
                            for sourceItem in sourceItems:
                                ...
                                d2 = sourceItem['num']
                                # 学制
                                d3 = sourceItem['length']
                                # 学费
                                d4 = f"{sourceItem['tuition']}/年"
                                # 选科要求 如果没有就是在sp里面
                            ...
                            if numNowFound >= numFound:
                                break
                            ItemPage += 1
                    with open(f"Data/{name}--{year}.csv", "a", newline="", encoding="UTF-8") as csvfile:
                        writer = csv.writer(csvfile)
